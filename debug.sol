...
contract ICOSale {
	...
	constructor(uint _initialPriceInWei,address _erc20TokenAddress, uint8 _decimal,uint _closeTime,uint _startTime,uint _minBuy,uint _maxBuy) public {
		governor = msg.sender;
	...
	function BuyWithWei(uint _amount) external payable {
		...
		require(_amount == msg.value,"Not Enough Wei To Buy!");
		require(closeTime < now,"Sale Is Now Closed!");
		require(startTime > now,"Sale Is Yet To Start!");
		uint _amounttosell = _amount.div(priceInWei).pow(decimal);
		require(_amounttosell > 0,"Error Overflow");
	...
	function WithdrawWei(uint _amount) external {
...
contract ICOFactory {
	...
	event NewICO(address _tokenAddress,ICOSale _c,address sender);
	function NewICOSale(uint _initialPriceInWei,address _tokenAddress,uint8 _decimal,uint _closeTime,uint _startTime,uint _minBuy,uint _maxBuy) public returns(address) {
		ICOSale c = new ICOSale(_initialPriceInWei,_tokenAddress,_decimal,_closeTime,_startTime,_minBuy,_maxBuy);
		emit NewICO(_tokenAddress,c,msg.sender);
		return address(c);
	}
